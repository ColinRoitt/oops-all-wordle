...
export default ({ setStatScreen }) => {
  ...
  const colors = grid.map((row, index) => convertToColors({ word: currentWord, row }));

  const barValues = [1, 2, 3, 4, 5, 6].map(
    (value) =>
      (gameFromLocalStorage.filter((game) => game.rounds === value).length / gamesPlayed) * 100
  );

  const clipboardContent = (showWord) =>
    [
      `Wordsle - ${showWord ? currentWord.toLowerCase() : ''} ${lastGame?.didWin ? round : 'X'}/6`,
      ...colors.map((row, index) =>
        row.reduce((acc, char) => (acc += char === 'Y' ? yellow : char === 'G' ? green : blank), '')
      )
    ].join('\n');
    ...
    <div className="stat-screen-cont" onClick={() => setStatScreen(false)}>
      <div className="stat-screen">
        ...
        <div className="numbers">
          ...
          <span className="value">
            <span className="num">{Math.round((gamesWon / gamesPlayed) * 100)}</span>
        ...
        <div className="graph">
          ...
          <div className="bars">
            {['one', 'two', 'three', 'four', 'five', 'six'].map((number, index) => (
              <div className={`${number} bar`} style={{ width: barValues[index] + '%' }}>
                <span className="val">
                  {gameFromLocalStorage.filter((game) => game.rounds === index + 1).length || '-'}
                </span>
              </div>
            ))}
          </div>
